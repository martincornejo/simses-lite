...
class TestDirectionReversal:
    def test_no_cycle_on_monotonic_charge(self):
        ...
        for soc in socs:
            assert det.step(soc, dt=60.0) is False
    ...
    def test_cycle_on_charge_then_discharge(self):
        ...
        det.step(0.6, dt=60.0)  # charge
        det.step(0.7, dt=60.0)  # charge
        result = det.step(0.65, dt=60.0)  # discharge -> reversal
    ...
    def test_cycle_on_discharge_then_charge(self):
        ...
        det = HalfCycleDetector(initial_soc=0.5)
        det.step(0.4, dt=60.0)  # discharge
        result = det.step(0.45, dt=60.0)  # charge -> reversal
        assert result is True
    ...
    def test_multiple_reversals(self):
        ...
        det = HalfCycleDetector(initial_soc=0.5)
        det.step(0.6, dt=60.0)  # charge
        assert det.step(0.55, dt=60.0) is True  # reversal 1
        assert det.step(0.6, dt=60.0) is True  # reversal 2
...
class TestStressFactors:
    def test_dod_calculation(self):
        ...
        det = HalfCycleDetector(initial_soc=0.3)
        det.step(0.5, dt=60.0)  # charge by 0.2
        det.step(0.45, dt=60.0)  # reversal
    ...
    def test_mean_soc(self):
        ...
        det.step(0.6, dt=60.0)  # midpoint avg = (0.4+0.6)/2 = 0.5
        det.step(0.55, dt=60.0)  # reversal
...
class TestFECAccumulation:
    def test_total_fec_accumulates(self):
        ...
        det = HalfCycleDetector(initial_soc=0.5)
        det.step(0.7, dt=60.0)  # charge
        det.step(0.6, dt=60.0)  # reversal 1: DOD=0.2, FEC=0.1
        fec1 = det.total_fec
        assert fec1 == pytest.approx(0.1)

        det.step(0.5, dt=60.0)  # continue discharge (no reversal)
        det.step(0.55, dt=60.0)  # reversal 2: DOD=0.1, FEC=0.05
...
class TestRestPeriods:
    def test_rest_does_not_trigger_cycle(self):
        ...
        det = HalfCycleDetector(initial_soc=0.5)
        det.step(0.6, dt=60.0)  # charge
        result = det.step(0.6, dt=60.0)  # rest
    ...
    def test_rest_does_not_affect_elapsed_time(self):
        ...
        det.step(0.7, dt=3600.0)  # charge 0.2 in 1h
        det.step(0.7, dt=7200.0)  # rest (should not count)
        det.step(0.65, dt=60.0)  # reversal
    ...
    def test_rest_between_movements_preserves_direction(self):
        ...
        det = HalfCycleDetector(initial_soc=0.5)
        det.step(0.6, dt=60.0)  # charge
        det.step(0.6, dt=60.0)  # rest
        det.step(0.7, dt=60.0)  # charge continues
        assert det.step(0.65, dt=60.0) is True  # reversal
        assert det.last_cycle.depth_of_discharge == pytest.approx(0.2)
